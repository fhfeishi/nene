// ============================================================================
// web/src/appv8.js - 博物馆RAG语音交互系统前端（完整修复版）
// ============================================================================
// 
// 修复内容：
...
// 5. requestId 比较修复：使用 String() 转换确保类型一致
// 6. sendMessage() 状态管理修复
//
// ## 修改后的数据流:
//
// 用户说话
...
// 后端STT识别 → final 结果
//     ↓
// 前端收到 final → hasSentCurrentRecognition = true
//     ↓
// sendRecognizedTextToRAG() → isGeneratingResponse = true
//     ↓
// sendToServer('send_message', {...}) → WebSocket 发送
//     ↓
// 后端处理 → response_start (带 requestId)
//     ↓
// 前端 handleResponseStart() → isSameRequestId() 比较（类型安全）
//     ↓
// 创建流式消息气泡 → 显示打字指示器
//     ↓
// 后端发送 response_chunk → 前端更新文字
//     ↓
// 后端发送 audio_chunk → 前端播放音频
//     ↓
// 后端发送 response_end → 前端重置状态
//     ↓
// hasSentCurrentRecognition = false，准备下一轮
//
// ============================================================================
...
class MuseumChatApp {
    constructor() {
        ...
        this.sessionId = null;                   // 会话ID（由后端分配或自动生成）
    ...
    initializeSocket() {
        ...
        this.socket.onopen = () => {
            ...
            console.log('📋 [Socket] 会话ID:', this.sessionId);
        ...
        this.socket.onmessage = (event) => {
            try {
                ...
                console.log('📨 [Socket] 收到消息:', data.type, 'requestId:', data.requestId);
    ...
    /**
     * 比较两个 requestId 是否相同（处理类型不一致问题）
     */
    isSameRequestId(id1, id2) {
    ...
    handleResponseStart(data) {
        ...
        console.log('🚀 [response_start] 收到, requestId:', requestId, '当前期望:', this.currentResponseRequestId);
        
        // 检查是否是当前请求的响应（使用宽松比较）
        if (requestId && this.currentResponseRequestId && 
            !this.isSameRequestId(requestId, this.currentResponseRequestId)) {
            console.log('⏭️ [忽略] 收到旧请求的响应，忽略');
            return;
        }

        console.log('✅ [response_start] 开始处理响应');
    ...
    handleResponseEnd(data) {
        ...
        this.currentResponseRequestId = null;    // 清空当前请求ID
    }
    ...
    handleError(data) {
        ...
        this.isGeneratingResponse = false;
        this.hasSentCurrentRecognition = false;
        this.currentResponseRequestId = null;
    ...
    handleRealtimeSpeechResult(data) {
        console.log('🔍 处理语音识别结果:', data.type, 
                    '已发送:', this.hasSentCurrentRecognition, 
    ...
    sendRecognizedTextToRAG(text) {
        ...
        console.log('🎤 [语音模式] 已启用, requestId:', requestId);
        ...
        console.log('📤 [发送RAG] 发送数据:', messageData);
        this.sendToServer('send_message', messageData);
    ...
    sendToServer(type, data) {
        if (this.socket && this.socket.readyState === WebSocket.OPEN) {
            ...
            this.showError('与服务器的连接已断开，请刷新页面重试');
        }
    }
    ...
    sendMessage() {
        ...
        const requestId = Date.now();
        this.currentResponseRequestId = requestId;
        this.isGeneratingResponse = true;  // ★★★ 修复：设置生成状态 ★★★
        ...
        console.log('📤 [sendMessage] 发送数据:', messageData);
        this.sendToServer('send_message', messageData);
